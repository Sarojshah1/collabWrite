...
const openai = env.OPENAI_API_KEY
  ...
  : null;
const gemini = env.GOOGLE_API_KEY
  ...
  try {
    ...
    if (env.AI_PROVIDER === "gemini") {
      ...
      try {
        ...
        while (attempts < 3) {
          try {
            // "gemini-flash-latest" alias usually points to a stable general purpose flash model
            ...
            // Throw 429 immediately so frontend can calculate delay if needed
            if (err.status === 429) {
              throw err;
            }
            if (err.status === 503 && attempts < 2) {
              attempts++;
...
${
  Array.isArray(outline) && outline.length
    ? `Outline to follow: ${outline.join(" | ")}`
    : ""
}`;
  ...
  try {
    ...
    if (!topic && !explanation) {
      return sendError(
        res,
        400,
        "Provide at least one of: topic or explanation"
      );
    }
...
- If keywords are provided, naturally incorporate them where appropriate.`;
    ...
    if (titleSuggestions.length < 2 && outline.length > 0) {
      const extra = outline
        ...
        .filter(Boolean);
      titleSuggestions = Array.from(
        new Set([...titleSuggestions, ...extra])
      ).slice(0, 5);
    }
  ...
  try {
    ...
    if (!userNeedPresent)
      return sendError(
        res,
        400,
        "Provide at least one of: prompt, topic, or explanation"
      );
    ...
    const user =
      ...
      (Array.isArray(keywords) && keywords.length
        ? `Prefer including these keywords where natural: ${keywords.join(
            ", "
          )}.\n`
        : "") +
      (Array.isArray(avoidWords) && avoidWords.length
    ...
    titles = Array.from(
      new Set(
        titles
          ...
          .map((t) =>
            t.length > maxLength
              ? t.slice(0, Math.max(0, maxLength - 1)).trimEnd() + "…"
              : t
          )
      )
    );
    ...
    if (titles.length < 2) {
      return sendError(
        res,
        502,
        "AI provider returned insufficient results. Try refining the topic/explanation and try again."
      );
    }
...
export const generateSummary = async (req, res) => {
  try {
    ...
    const system =
      "Summarize the content in plain English, focusing on key points. Keep it under the requested word limit.";
...
export const generateKeywords = async (req, res) => {
  try {
    ...
    const system =
      "Extract SEO keywords and keyphrases from the content. Return as a JSON array of strings only.";
...
export const generateOutline = async (req, res) => {
  try {
    ...
    const system =
      "Create a reasonable blog outline with H2/H3 structure. Return as a JSON array of section titles.";
    const user = prompt;
