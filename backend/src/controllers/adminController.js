...
export const suspendUser = async (req, res) => {
  try {
    ...
    if (typeof suspended !== "boolean")
      return sendError(res, 400, "suspended must be boolean");
...
export const deleteUser = async (req, res) => {
  try {
    ...
    if (blogCount > 0)
      return sendError(res, 400, "User has blogs; suspend instead of delete");
...
export const analytics = async (req, res) => {
  try {
    ...
    const activeAuthorDetails = await User.populate(activeAuthors, {
      path: "_id",
      select: "name email",
    });
            ...
            $sum: {
              $switch: {
                branches: [
                  ...
                  {
                    case: { $eq: ["$type", "bookmark"] },
                    then: weights.bookmark,
                  },
                  {
                    case: { $eq: ["$type", "comment"] },
                    then: weights.comment,
                  },
