...
export function initCollaborationSocket(io) {
  io.on("connection", (socket) => {
    socket.on("joinSession", async ({ blogId, userId, cursor, userInfo }) => {
      if (!blogId || !userId)
        return socket.emit("error", { message: "Missing blogId or userId" });
      socket.join(`blog:${blogId}`);
      joinPresence(blogId, userId, cursor || null, userInfo);
    ...
    socket.on("paragraphEdit", async ({ blogId, userId, segmentId, text }) => {
      try {
        if (!blogId || !userId || !segmentId || typeof text !== "string")
          return;
        ...
        console.log("[collab] paragraphEdit", {
          blogId,
          userId,
          segmentId,
          hasPrev: !!prev,
    ...
    socket.on("editContent", async ({ blogId, userId, delta }) => {
      try {
        ...
        socket
          .to(`blog:${blogId}`)
          .emit("editContent", { blogId, userId, delta });
      } catch (err) {
