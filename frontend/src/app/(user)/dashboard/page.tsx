  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        setError(null);
        const data = await listBlogs({ status: statusFilter === "all" ? undefined : statusFilter, sort: "newest" });
    ...
    <section className="mx-auto max-w-[1200px] px-6 py-8">
      <header className="mb-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <h1 className="text-3xl font-semibold text-zinc-900">Your Writing Dashboard</h1>
            <p className="mt-1 text-sm text-zinc-600">Track performance, reopen drafts, and enhance collaboration.</p>
          </div>
            ...
            <div className="relative w-full sm:w-[360px]">
              ...
              {showSuggestions && (
                <div className="absolute z-20 mt-2 w-full overflow-hidden rounded-2xl border border-zinc-200 bg-white py-1 text-sm shadow-lg">
                  <p className="px-4 py-2 text-[11px] font-medium uppercase tracking-wide text-zinc-500">Suggestions</p>
                  <ul className="max-h-64 divide-y divide-zinc-100 overflow-auto">
                    {suggestionBlogs.map((b) => (
                      <li key={b._id}>
                        <a href={`/blog/${b._id}`} className="flex items-center justify-between gap-3 px-4 py-2 hover:bg-zinc-50">
                          <div className="min-w-0">
                            <p className="truncate text-sm font-medium text-zinc-900">{b.title}</p>
                            {typeof b.author === "object" && b.author && "name" in b.author && (
                              <p className="truncate text-[11px] text-zinc-500">{(b.author as any).name as string}</p>
                            )}
                          </div>
                          <p className="shrink-0 text-[11px] text-zinc-400">
                            {(b.updatedAt || b.createdAt) && new Date(b.updatedAt || b.createdAt).toLocaleDateString()}
                          </p>
                        </a>
                      </li>
                    ))}
        ...
        <div className="mt-6 flex flex-wrap items-center gap-2">
          <button
            ...
            className={`inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-medium ring-1 ring-inset ring-zinc-200 ${
              statusFilter === "published" ? "bg-zinc-100 text-zinc-900" : "bg-white text-zinc-700 hover:bg-zinc-50"
            }`}
          >
          ...
          <button
            ...
            className={`inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-medium ring-1 ring-inset ring-zinc-200 ${
              statusFilter === "draft" ? "bg-zinc-100 text-zinc-900" : "bg-white text-zinc-700 hover:bg-zinc-50"
            }`}
          >
          ...
          <button
            ...
            className={`inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-medium ring-1 ring-inset ring-zinc-200 ${
              statusFilter === "all" ? "bg-zinc-100 text-zinc-900" : "bg-white text-zinc-700 hover:bg-zinc-50"
            }`}
          >
          ...
          <div className="space-y-4">
            {filteredBlogs.map((b) => (
              <BlogCard
                key={b._id}
                title={b.title}
                excerpt={b.contentHTML ? b.contentHTML.replace(/<[^>]+>/g, " ").slice(0, 120) + "..." : undefined}
                ...
                collaborators={
                  typeof b.author === "object" && b.author && "name" in b.author
                    ? [
                        { name: b.author.name, avatar: (b.author as any).avatar || undefined },
          ...
          <aside className="hidden lg:block">
            <div className="sticky top-6 space-y-4">
              <div className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm">
                <h2 className="text-sm font-semibold text-zinc-900">Dashboard Insights</h2>
                <div className="mt-3 divide-y divide-zinc-100">
                  <div className="flex gap-3 py-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-zinc-900">2 posts need review</p>
                      <p className="text-xs text-zinc-500">Left unanswered for 7+ days</p>
                    </div>
                  </div>
                  <div className="flex gap-3 py-3">
                    <span className="mt-0.5 inline-flex h-6 w-6 items-center justify-center rounded-full bg-zinc-100 text-zinc-700">
                    ...
                    <div>
                      <p className="text-sm font-medium text-zinc-900">1 draft hasn't been updated</p>
                      <p className="text-xs text-zinc-500">Last edited 3 weeks ago</p>
                    </div>
                  </div>
                  <div className="flex gap-3 py-3">
                    <span className="mt-0.5 inline-flex h-6 w-6 items-center justify-center rounded-full bg-zinc-100 text-zinc-700">
                    ...
                    <div>
                      <p className="text-sm font-medium text-zinc-900">Short titles perform better</p>
                      <p className="text-xs text-zinc-500">Consider revising "Testing Second"</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm">
                ...
                <div className="mt-3 space-y-2">
                  <a href="/dashboard/write" className="flex items-center gap-2 rounded-xl px-3 py-2 text-sm text-zinc-700 hover:bg-zinc-50">
