...
function loadToggles(): ToggleState {
  ...
  try {
    ...
    return raw ? (JSON.parse(raw) as unknown as ToggleState) : {};
  } catch {
    return {};
  }
}
...
const DESCRIPTIONS: Record<ToggleKey, string> = {
  darkPage:
    "Start documents with a darker page background to reduce eye strain.",
  showCursors: "Display other editors' cursors and initials in real time.",
  aiHints:
    "Allow the editor to surface AI tips and merge suggestions while you write.",
  emailAlerts: "Receive updates when a collaborator assigns you new work.",
  commentNotifications:
    "Show in-app badges when someone comments on your document.",
  analyticsOptIn:
    "Anonymously aggregate usage to power better Reports & Analytics.",
};
    ...
    <section className="mx-auto max-w-5xl px-4 sm:px-6 py-6 space-y-6">
      <header className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-xs sm:text-sm text-zinc-600">
            Tune how CollabWrite looks and behaves for you. These preferences
      ...
      <article className="rounded-2xl border border-zinc-200 bg-white/90 p-4 shadow-sm flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-[11px] text-zinc-600">
            Manage your account details from the Profile tab. Settings here only
            affect this device.
          </p>
        </div>
        <div className="flex items-center gap-3 text-xs text-zinc-600">
          ...
          <div className="hidden sm:block text-right">
            ...
            <p className="text-[11px] text-zinc-500">
              Open the Profile page to edit name or avatar.
            </p>
          </div>
        </div>
      </article>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        ...
        <article className="rounded-2xl border border-zinc-200 bg-white/90 p-4 shadow-sm flex flex-col gap-3">
          ...
          <p className="mb-3 text-[11px] text-zinc-600">
            Choose how much real‑time activity and AI assistance you want to see
            while collaborating.
          </p>
          <div className="space-y-3">
            {toggleOrder
        ...
        <article className="rounded-2xl border border-zinc-200 bg-white/90 p-4 shadow-sm flex flex-col gap-3">
          ...
          <p className="mb-3 text-[11px] text-zinc-600">
            Control alerts and how your anonymous usage contributes to dashboard
            insights.
          </p>
          <div className="space-y-3">
            {toggleOrder
